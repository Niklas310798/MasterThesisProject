...
def handle_pkt(pkt):
    ...
    threshold_dict_key = "tstamp_latency_over_{}ms".format(trigger)
    tmp_stats = {}
    if tmp_name in statsHandler.getStats():
        ...
        tmp_stats[threshold_dict_key] = 0
        tmp_stats["events"] = []
    ...
    if (int_info["tstamp_latency"] > 150):
    ...
    if (len(tmp_deque) > 30):
        avg_latency = dequeHandler.getDequeAverage(tmp_name)
        # print(avg_latency)
        trigger_msg = None
